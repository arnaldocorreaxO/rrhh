...
class MarcacionListView(PermissionRequiredMixin, ListView):
	model = Marcacion
	template_name = 'as/marcacion/list.html'
	...
	def post(self, request, *args, **kwargs):
		...
		try:
			if action == 'load_data':
				marcacion = Marcacion.objects.get(id=marcacion_id)                
				...
				for i in qs:
					...
					item['hora'] = item['hora'].strftime('%H:%M:%S')
					# item['reloj_str'] = f"{item['reloj__denominacion']} - {item['reloj__ip']}"
					if item['archivo']:
						item['archivo'] = '{0}{1}'.format(settings.MEDIA_URL, item['archivo'])
